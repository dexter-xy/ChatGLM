...
export const generateTitle = async (
	...
	console.log('Ollama Title Generation - Template:', template);
	console.log('Ollama Title Generation - Model:', model);
	...
	const res = await fetch(`${OLLAMA_API_BASE_URL}/api/generate`, {
		...
		.catch((err) => {
			console.log('Ollama Title Generation - Error:', err);
			if ('detail' in err) {
				error = err.detail;
			}
			return null;
	...
	console.log('Ollama Title Generation - Response:', res);
	
	const titleContent = res?.response;
	console.log('Ollama Title Generation - Raw Content:', titleContent);
	
	if (!titleContent || typeof titleContent !== 'string') {
		console.log('Ollama Title Generation - No valid content, returning default');
		return 'New Chat';
	}

	const cleanTitle = titleContent.replace(/["']/g, '').trim();
	console.log('Ollama Title Generation - Clean Title:', cleanTitle);
